function g = diskfun_new(f, varargin)
...
%try speeding up evaluation: 
ii = (1:length(r)); 
jj = (1:length(lambda)).'; 
%rr = repmat(r(ii), 1, length(lambda)); 
%tt = repmat(theta(ii), 1, length(lambda)); 
%ll = repmat(lambda.', 1, length(r)); 

%G2 = bsxfun(@(jj,ii) fevalm(f,r(ii), lambda(jj), theta(ii)), jj,ii); 
G2 = bsxfun(@(jj,ii) feval(f,r(ii), lambda(jj), theta(ii), 'polar'), jj,ii); 
