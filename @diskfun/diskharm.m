%diskharm(L,m,type)
%analogous to sphharm, gives basis functions for expansions on the unit
%disk 
%function returned is the Lth order, mth eigenfunction (e.g. (J_L(w_m r)*cos(L*theta) ) 
%for laplacian with
    ...
    function   Z=mydiskharm(theta, r, L, m, type)
       ...
       if type=='N'
           if (L==0 && m==1)
            ...
            jzero = roots(chebfun(@(x) L*besselj(L,x)-x.^(0^(0^L)).*besselj(L+1, x), [L (m+L/2)*pi])); %the weirdness x^(0^0^L) accounts for the fact that the x-term is not present when L=0
