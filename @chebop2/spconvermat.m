function T = spconvermat(n,lam,k)
% Returns the conversion matrix, that converts C^{lam} (Ultraspherical 
% polynomials) to C^{lam+k}, If k=0, returns identity. 

T=speye(n);
for s = lam:lam + k - 1
    T = spconvert(n,s)*T;
end
end

function T = spconvert(n,lam)
% CONVERMAT(N,LAM) This computes the truncation of the operator that
% transforms C^{lam} (Ultraspherical polynomials) to C^{lam+1}.  The
% truncation gives back a matrix of size n x n.

%Relation is: C_n^(lam) = (lam/(n+lam))(C_n^(lam+1) - C_{n-2}^(lam+1))
if(lam==0)
    dg = .5*ones(n-2,1);  
%     T = spdiags([1;.5;dg 0;0;-dg],[0 2],n,n);
    T = spdiags([1 0; .5 0; dg -dg],[0 2],n,n);
else
    dg = lam./(lam + (2:n-1))';
    T = spdiags([1 0;lam./(lam+1) 0; dg -dg],[0 2],n,n);
end

end