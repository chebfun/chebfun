...
%    METHOD = 'GLR' uses the Glaser-Liu-Rokhlin fast algorithm [2], which is 
%     fast and can give better relative accuracy for the -.5<X<.5 than 'ASY' 
%     (although the accuracy of the weights is usually worse).
%    METHOD = 'GW' will use the traditional Golub-Welsch eigenvalue method, 
%       which is maintained mostly for historical reasons.
%
%   [X, W, V, T] = LEGPTS(N) returns also the cosine of the nodes, T = cos(X).
...
if ( nargin > 1 )
    ...
    validStrings = {'default', 'GW', 'fast', 'fastsmall', 'GLR', 'ASY', 'REC'};
...
function [x, w, v, ders] = alg0_Leg(n) % Driver for GLR
    ...
    % Call main algorithm (alg1_Leg) for other roots and derivatives:
    [x, ders] = alg1_Leg(x,ders);          
...
function [roots, ders] = alg1_Leg(roots, ders)  % Main algorithm for GLR
    ...
    % Define some constants: (We start in the middle, at N around n/2)
    n = length(roots);
    s = mod(n, 2);
    N = (n - s) / 2;
    ...
    theta = pi*(4*k-1)/(4*n+2);
    roots(((n+4-s)/2):end) = ...
        ( 1 - (n-1)/(8*n^3) - 1/(384*n^4)*(39-28./sin(theta).^2) ).*cos(theta);
    x = roots(N+1);
...
function [x1, d1] = alg2_Leg(Pn0,n) % Find the first root (note P_n'(0)==0)
    ...
    % Approximate first root via asymptotic formula: (Tricomi)
    k = ceil(n/2); 
    theta = pi*(4*k-1)/(4*n+2);
...
function [x, w, v, t, ders] = asy1(n, nbdy)
    ...
    while norm(dt,inf) > sqrt(eps)/1000
        ...
        j = j + 1;
        dt = dt(1:idx-1);
        if ( j > maxj )                        % Too many iterations. Give up.
            dt = 0; 
        end
    end
...
%     if flag
        ...
        % Compute cosAlpha(k,:) and sinAlpha(k,:):
        sint = sin(t); 
        cost = cos(t);
        for k = 2:M
...
function Ja = besseltaylor(t, z)
    ...
    Bjk = besselj(nu, JK, 0);
    % nchoosek    
    nck = abs(pascal(floor(1.25*kmax), 1)); nck(1,:) = []; 
...
function [tB1, A2, tB2, A3, tB3, A4] = asy2_higherterms(a, b, theta, n)
    ...
    % The constants a = alpha and b = beta, but these are more useful:
    A = (.25-a^2); 
    B = (.25-b^2);
    ...
    B1(1) = A/720 + A^2/576 + A*B/96 + B^2/64 + B/48 + ...
        a*(A^2/576 + B^2/64 + A*B/96) - a^2*(A/720 + B/48);
