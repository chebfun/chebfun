...
% Q = CUMSUMMAT(N) is the matrix that maps function values at N Chebyshev points
...
k = 1:Nm1;
k2 = 2*(k-1);
k2(1) = 1;          % Avoid divide by zero
B = diag(1./(2*k),-1) - diag(1./k2,1);
v = ones(Nm1, 1);
v(2:2:end) = -1;
B(1,:) = sum( diag(v)*B(2:Nm1+1,:), 1 );
B(:,1) = 2*B(:,1);

% Form the CUMSUM matrix:
Q = T*B*Tinv;
