...
%   and OP is a function handle. Note that the inputs are assumed to "make 
%   sense", that is, F and G are assumed to have the same domain. No warning 
%   will be given/error thrown if that is not the case, but the output of the 
...
%   of G, an error is thrown. Notice that the domain of H will be the same as 
%   the domain of F.
%
%   H = COMPOSE(F, OP, G, PREF) or H = COMPOSE(F, OP, [], PREF), where F (and
%   G) are UNBNDFUN objects, and OP is a function handle, uses the options 
%   passed by the preferences structure PREF to build the returned UNBNDFUN. In
...
if ( nargin == 2 )
    if ( isa(op, 'unbndfun') )  
        ...
        % For composition to work with two FUN objects, the range of F must
...
elseif ( nargin == 3 )          % out = OP(F,G). No preferences passed.
    ...
    % Third argument passed must have been an UNBNDFUN. Compose the ONEFUN of f
