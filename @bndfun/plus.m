...
elseif ( isa(g, 'double') ) % BNDFUN + double
    ...
    % Update the onefun, everything else stays the same. The onefun plus
    % method ensures that the dimensions of the BNDFUN argument and the
    % double argument match (that is, their number of columns):
    f.onefun = f.onefun + g; 
...
elseif ( isa(f,'double') ) % double + FUNCHEB
    ...
    % Domains match, and since we're only working with BNDFUNs, their
    % linear mappings must match as well. Hence, we just need to add the
    % onefuns:
