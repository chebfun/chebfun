function out = poly(f)
%POLY	Polynomial coefficients of a FUNCHEB1.
%   C = POLY(F) returns the polynomial coefficients of F so that 
%           F(x) = C(N+1)*x^N + C(N)*x^(N-1) + ... + C(2)*x + C(1)
%   
%   Note that unlike the MATLAB POLY command, FUNCHEB1/POLY can operate on
%   vector-valued FUNCHEB1 objects, and hence product a matrix output. In such
