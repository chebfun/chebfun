...
if ( (ndimsx > 2) || (sizex(2) > 1) )
    if ( m > 1)
        warning('CHEBFUN:FUNCHEB1:bary:xcal', ...
            'First input to BARY should be a column vector.');
    end
    x = x(:);
end
...
% Chebyshev nodes of 1st kind and the corresponding barycentric weights
...
if ( numel(x) < length(xk) )  % Loop over evaluation points
    ...
    % Loop:
    for j = 1:length(xk),
%         tmp = repmat(ek(j) ./ (x - xk(j)), 1, m);
        tmp = (ek(j) ./ (x - xk(j)))*onevec;
        num = num + (tmp*diag(gvals(j,:)));
        denom = denom + tmp;
    end
    fx = num ./ denom;
end
