...
%ROOTS	Roots of a FUNCHEB1 in the interval [-1,1]
%   ROOTS(F) returns the real roots of the FUNCHEB1 F in the interval [-1,1].
%
...
% funcheb1 is of degree less than 50, at which point a companion matrix is 
...
% Support for vectorised FUNCHEB1 objects.
...
function r = rootsunit_coeffs ( c , htol  )
    ...
    % c on the unit interval.
        ...
        % Here the we choose 512, instead of 513, since this may facilitate
        % the FFTs.
        ...
        elseif ( n <= 512)
            ...
            if isempty( Tleft )
                ...
                w = repmat((2/512)*exp(-1i*(0:511)*pi/(2*512)).',1,512);
                Tleft = [Tleft(1:2:511,:); Tleft(512:-2:2,:)];
                Tleft = real(w.*fft(Tleft));
                
                % Halve the constant term in the Chebyshev series:
                Tleft(1,:) = Tleft(1,:)/2;
                norm(abs(Tleft-triu(Tleft)),inf)
                % Clean up round off errors
                Tleft = triu(Tleft);
                
                % create the coefficients for Tright much in the same way
                x = chebptsAB(512, [splitpoint,1]);
                x = flipud(x);
                Tright = ones(512);
                Tright(:,2) = x;
                for k = 3:512
                ...
                Tright = [Tright(1:2:511,:); Tright(512:-2:2,:)];
                Tright = real(w.*fft(Tright));
        ...
        % Otherwise, split using more traditional methods
            ...
            v = funcheb1.clenshaw( [ chebptsAB(n, [-1, splitpoint]) ; chebptsAB(n, [splitpoint, 1]) ], c(end:-1:1) );

            % Get the coefficients on the left
            cleft = funcheb1.chebpoly(v(1:n));
            cleft = cleft(end:-1:1);

            % Get the coefficients on the right
            cright = funcheb1.chebpoly(v(n+1:end));
