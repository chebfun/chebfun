...
%QR     QR factorisation of a multivalued FUNCHEB1.
%   [Q, R] = QR(F) returns a QR factorisation of F such that F = Q*R, where the
%   FUNCHEB1 Q is orthogonal (wrt the continuous L^2 norm on [-1,1]) and of the
...
xc = funcheb1.chebpts(n);
[xl, wl, vl] = legpts(n);
P = funcheb1.barymat(xl, xc);
...
Pinv = funcheb1.barymat(xc, xl, vl); % Revert to Chebyshev grid (from Legendre).
S = spdiags(sign(diag(R)), 0, m, m);    % Enforce diag(R) >= 0.
Q = Pinv*Winv*Q*S;          % Fix Q.
R = S*R;                    % Fix R.

f.values = Q;                           % Adjust values of f.
f.coeffs = funcheb1.chebpoly(Q);        % Compute new coeffs.
