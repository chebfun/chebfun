...
if ( isa(f, 'double') )                
    ...
    if ( (nargin >= 3) && isa(argin{1}, 'double') && isa(argin{2}, 'fourfun2') )
        % Extract inputs:
        xx = f; 
        yy = argin{1}; 
        f = argin{2};
...
elseif ( isa(f, 'fourfun2') ) 
    
    dom = f.domain;
    if ( (nargin == 3) || (nargin > 3) && ~isa(argin{1},'fourfun2') ) 
        % CONTOUR(xx, yy, f)
    ...
    elseif ( (nargin >= 3) && isa(argin{1},'fourfun2') && isa(argin{2},'fourfun2') )
        ...
        if ( ~domainCheck(xx, yy) || ~domainCheck(yy, f) )
            error('CHEBFUN2:CONTOUR:DOMAIN', ...
                'Domains of fourfun2 objects are not consistent.');
        end
