...
if ( isa(f1, 'fourfun2') && isa(f2, 'fourfun2') )  % quiver(f,g)
    ...
    elseif ( nargin == 3 )                         % quiver(f,g,S)
        ...
        if ( isa(varargin{1}, 'fourfun2v') )       % quiver(x,y,F)
            varargin{2} = {};
            [xx, yy] = new_data_locations(f1, f2, numpts);
            % Call CHEBFUN2V/QUIVER().
            h = quiver( xx, yy, varargin{:} );   
        elseif ( ~isa(varargin{1}, 'fourfun2') )   % quiver(x,y,S) or quiver(x,y,'linespec')
            h = quiver( [f1;f2], varargin{:} );
        else
            error('CHEBFUN2:QUIVER:INPUT', ...
                'Stretching parameter should not be a fourfun2 object.');
        end
    ...
    elseif ( nargin > 3 )
        ...
        f3 = varargin{1}; 
        f4 = varargin{2};
        if (isa(f3, 'fourfun2') && isa(f4, 'fourfun2') ) % quiver(x,y,f,g,...)
