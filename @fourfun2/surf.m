...
if ( isa(f,'fourfun2') )
    ...
    if ( (nargin == 1) || ...
            ( (nargin > 1) && ~isempty(argin) && ~isa(argin{1}, 'fourfun2') ) || ...
            ( (nargin == 3) && isempty(argin)) )
        % surf(f,...)
        
        dom = f.domain;
        x = fourfun2(@(x,y) x,dom); 
        y = fourfun2(@(x,y) y,dom);
    ...
    elseif ( nargin > 2)                    
        ...
        x = feval(x, xx, yy); 
        y = feval(y, xx, yy);
        if ( isa(argin{2}, 'fourfun2') )         % surf(x,y,f,...)
            ...
            elseif ( isa(argin{3}, 'double') )   % surf(x,y,f,C,...)
                C = argin{3};
                argin(3) = [];
            elseif ( isa(argin{3}, 'fourfun2'))  % Colour matrix is a CHEBFUN2.
