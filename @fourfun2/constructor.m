...
if ( isa(op, 'fourfun2') )  % CHEBFUN2( CHEBFUN2 )
    g = op;
    return
end
...
if ( vectorize == 0 ) % another check
    ...
    if ( any(any( abs(A - B.') > min( 1000*pseudoLevel, 1e-4 ) ) ) )
        ...
        % then vectorize.
        warning('CHEBFUN2:CTOR:VECTORIZE','Function did not correctly evaluate on an array. Turning on the ''vectorize'' flag. Did you intend this? Use the ''vectorize'' flag in the fourfun2 constructor call to avoid this warning message.');
        g = fourfun2(op, domain, 'vectorize');
        return
    end
end
...
function dom = getDefaultDomain( technology )
