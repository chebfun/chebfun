...
if ( isa(F, 'fourfun2v') )              % quiver(Z,F,...)
    zz = new_data_locations(Z, numpts);
    h = quiver3(zz, F, varargin{:});
elseif ( nargin > 3 )
    if ( isa(varargin{2}, 'fourfun2v') ) % quiver(X,Y,Z,F,...)
    ...
    elseif ( isa(Z, 'fourfun2') && isa(F, 'fourfun2') &&...
            isa(varargin{1}, 'fourfun2') &&...
            isa(varargin{2}, 'fourfun2') &&...
            ( nargin ==4 || ~isa(varargin{3}, 'fourfun2')) )
        FF = vertcat( vertcat(F, varargin{1}), varargin{2}) ;
        h = quiver3( Z, FF );     % call quiver3(Z,F,...)
    elseif ( ( nargin > 5 ) && isa(Z,'fourfun2') && isa(F,'fourfun2') &&...
            isa(varargin{1}, 'fourfun2') &&...
            isa(varargin{2}, 'fourfun2') &&...
            isa(varargin{3}, 'fourfun2') &&...
            isa(varargin{4}, 'fourfun2') &&...
            ( nargin == 6 || ~isa(varargin{5}, 'fourfun2')) )
