...
if ( n == 0 )
    if(~iscell(f))
        ...
        q = chebfun(1,[dom{1}(1),dom{end}(end)]);
    end
end
...
h = -inf; % = -1;
err = normf; % TODO: this will cause problems if normf = 1!!!
interpSuccess = 1;
    ...
    if ( n == 0 )
        ...
        if(interpSuccess == 1)
            ...
            if(~iscell(f))
                ...
                % TODO: see if there is anything to do in the
                % multi-interval case
            end
        end
    end
...
if (~iscell(f))
    ...
    opts.dom = f{1}.domain([1,end]);
    opts.normf = norm(f{1});
    for ii = 2:length(f)
        opts.dom = {opts.dom, f{ii}.domain([1,end])};
        newNorm = norm(f{ii});
        if(newNorm > opts.normf)
            opts.normf = newNorm;
        end
    end
end
...
% TODO: need to think about if this applies to the multi-interval case
...
if(~iscell(f))
    ...
    for ii = 1:length(f)
        ...
        if ismember(f{ii}.domain(end),xki) == 0
            xadd = [(f{ii}.domain(end)+xki(end))/2;xadd];
        end
    end
end
...
if(~iscell(f)) 
    ...
    p = chebfun(pvals,[f{1}.domain(1),f{end}.domain(end)]);
    q = chebfun(qvals,[f{1}.domain(1),f{end}.domain(end)]);
end
...
if (~iscell(f))
    ...
    if(n == 0)
        ...
        for ii = 1:length(f)
            ...
            xki = xki(xki <= f{ii}.domain(end));
            xki = unique([xki; f{ii}.domain']);
            xki = findExtrema(f{ii},rh,sort(xki,'ascend'));
            xki = unique([xki; f{ii}.domain']);
            xki = sort(xki,'ascend');
            err_handle = @(x) feval(f{ii}, x) - rh(x);
            rr = [rr; xki];
