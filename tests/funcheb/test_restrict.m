...
for ( n = 1:2 )
    ...
    try
        ...
        pass(n, 3) = strcmp(ME.identifier, 'FUNCHEB:restrict:badinterval');
    end
    ...
    try
        ...
        pass(n, 4) = strcmp(ME.identifier, 'FUNCHEB:restrict:badinterval');
    end

    try
        g = restrict(f, [-1 -0.25 0.3 0.1 1]);
        pass(n, 5) = 0;
    catch ME
        pass(n, 5) = strcmp(ME.identifier, 'FUNCHEB:restrict:badinterval');
    end
...
function result = test_spotcheck_restrict(testclass, fun_op, subint, pref)
    ...
    result = norm(y_exact - y_approx, Inf) < 10*g.epslevel;
end
