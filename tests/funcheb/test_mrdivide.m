% Test file for funcheb/mrdivide.
...
for ( n = 1:2 )
    ...
    %%
    % Check division of a funcheb object by a numeric array.
    ...
    %%
    % Check division of a numeric array by a funcheb object.
    ...
    try
        ...
        pass(n, 7) = strcmp(ME.identifier, 'CHEBFUN:FUNCHEB:mrdivide:size');
    end
    
    % Can't do f/g if both f and g are funcheb objects.
    try
        f = testclass.make(@(x) sin(x));
        ...
        pass(n, 8) = strcmp(ME.identifier, 'CHEBFUN:FUNCHEB:mrdivide:funfun');
    end
    
    % Can't call mldivide on a funcheb and a non-funcheb or non-double 
    % object.
    try
        f = testclass.make(@(x) sin(x));
        g = f / true;
        ...
        pass(n, 9) = strcmp(ME.identifier, 'CHEBFUN:FUNCHEB:mrdivide:derp');
    end
end
