...
for ( n = 1:2 )
    ...
    pass(n, 3) = size(g,1) == 2 && norm(g.values - sin(1)*[-1 ; 1], inf) < tol;
    ...
    % [TODO]:  This test fails for funcheb1.
    pass(n, 6) = size(g,1) == 2 && ...
        norm(g.values - sin(1)*[-1 1 ; 1 -1], inf) < tol;
    ...
    g = prolong(f,1);
    % [TODO]:  This test fails for funcheb1.
    pass(n, 12) = size(g, 1) == 1 && norm(g.values, inf) < tol;
    
    g = prolong(f,2);
    % [TODO]:  This test fails for funcheb1.
    pass(n, 13) = size(g, 1) == 2 && ...
        norm(g.values - sin(1000)*[-1 1 ; 1 -1], inf) < tol;
    ...
    pass(n, 15) = length(g) == 2 && ...
        norm(g.values - cos(1000)*[1 -1 ; 1 -1], inf) < tol;
