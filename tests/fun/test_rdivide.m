...
pass = zeros(1, 16); % Pre-allocate pass matrix
for n = 1:1 %[TODO]: unbndfun
    ...
    pass(n, 5) = norm(feval(g, x) - g_exact(x), inf) < 10*max(g.onefun.vscale)*g.onefun.epslevel;
    ...
    try
        ...
        g = testclass.make(@(x) sin(x), dom);
        disp(f ./ g);
        pass(n, 14) = false;
    catch ME
        pass(n, 14) = strcmp(ME.identifier, ...
            'CHEBFUN:FUN:rdivide:DivideByZeros');
    end
...
function result = test_div_function_by_scalar(f, f_op, alpha, x)
    ...
    result = norm(feval(g, x) - g_exact(x), inf) < 10*max(g.onefun.vscale)*g.onefun.epslevel;
end
...
function result = test_div_function_by_function(f, f_op, g, g_op, x)
    ...
    result = norm(feval(h, x) - h_exact(x), inf) < 10*max(h.onefun.vscale)*h.onefun.epslevel;
end