...
pass(2) = ~isempty(ind) && (h.impulses(ind) == 0) && ...
    all(feval(h, x) - heaviside(sqrt(2)/2 - sin(x)) == 0);
...
pass(3) = ~isempty(ind1) && (h.impulses(ind1) == 0) && ...
    ~isempty(ind2) && (h.impulses(ind2) == 0) && ...
    all(feval(h, x) - heaviside(g_op(x) - exp(x)) == 0);
...
ind1 = find(abs(h.domain - 0.5) < 10*vscale(h)*epslevel(h));
ind2 = find(abs(h.domain + 0.5) < 10*vscale(h)*epslevel(h));
pass(6) = ~isempty(ind1) && (h.impulses(ind1) == 0) && ...
    ~isempty(ind2) && (h.impulses(ind2) == 0) && ...
    all(feval(h, x) - heaviside(g_op(x) - exp(x)) == 0);
...
try
    ...
    pass(7) = strcmp(ME.identifier, 'CHEBFUN:lt:array');
end
...
try
    ...
    pass(8) = strcmp(ME.identifier, 'CHEBFUN:lt:array');
end
