...
lval2 = feval(f, 'start');
lval3 = feval(f, '-');
pass(2) = (abs(lval1 - erf(-1)) < 10*eps*f.vscale ) ...
    && isequal(lval1, lval2, lval3);

rval1 = feval(f, 'right');
rval2 = feval(f, 'end');
rval3 = feval(f, '+');
pass(3) = (abs(rval1 - erf(1)) < 10*eps*f.vscale ) ...
...
x = xr;
pass(10) = (norm(feval(f, x) - f_exact(x), inf) < 10*eps*f.vscale);

pref.splitting = 1;
f_exact = @(x) abs(x)./(1 + x.^2);
f = chebfun(f_exact, [-2 7], pref);
x = 4.5*xr + 2.5;
pass(11) = (norm(feval(f, x) - f_exact(x), inf) < 1e2*eps*f.vscale);
...
x = 2*xr + 3;
pass(12) = (norm(feval(f, x) - f_exact(x), inf) < 1e5*eps*f.vscale);
...
f = chebfun(f_exact, [-1 1], pref);
x = xr;
pass(13) = (norm(feval(f, x) - f_exact(x), inf) < 10*eps*f.vscale);
...
err = feval(f, x.') - f_exact(x.');
pass(20) = norm(err(:), inf) < 10*eps*f.vscale;
...
x = xr;
err = feval(f, x) - f_exact(x);
pass(21) = all(max(abs(err)) < 1e2*eps*f.vscale);
...
pass(26) = norm(err(:), inf) < 10*eps*f.vscale;

err = feval(f, 'right') - [sin(1) 1 exp(1i)].';
pass(27) = norm(err(:), inf) < 10*eps*f.vscale;

% Test feval(chebfun, location, 'left'/'right'):
f = chebfun({@(x) [x, 2*x], @(x) [3*x, 4*x]}, [0 1 2]);
pass(28) = norm(feval(f, 1, 'left') - [1, 2], inf) < 10*eps*f.vscale;
pass(29) = norm(feval(f, 1, 'right') - [3, 4], inf) < 10*eps*f.vscale;
...
x = [-1 ; 0.5 ; 1];
err = feval(f, x) - f_exact(x);
pass(30) = all(max(abs(err)) < 10*eps*f.vscale);
