...
Q = legpoly(0:4);
pass(1) = norm(legcoeffs(Q) - eye(5), 'fro') < 10*vscale(Q)*eps;

% Check coefficients of a linear combination of Legendre polynomials.
Q = legpoly(0:9);
v = (1:1:10).';
Qv = Q*v;
pass(2) = norm(legcoeffs(Qv) - v, Inf) < 10*vscale(Qv)*eps;
...
pass(3) = norm(c(:) - c_exact(:), Inf) < 10*vscale(f)*eps;
...
c_exact = [ 0.5450000000;
           -0.4365000000;
            ...
            0.2173762500;
           -0.0574494375; ];                                
pass(4) = norm(c - c_exact, Inf) < 10*vscale(f)*eps;
...
pass(5) = norm(c(:) - c_exact(:), Inf) < 10*vscale(f)*eps;

% Check operation for row CHEBFUNs.
c = legcoeffs(f.', 5);
pass(6) = norm(c(:) - c_exact(:), Inf) < 10*vscale(f)*eps;
