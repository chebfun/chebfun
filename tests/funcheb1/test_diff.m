...
f = funcheb1(@(x) exp(x) - x, pref);
df = diff(f);
df_exact = @(x) exp(x) - 1;
err = df_exact(x) - feval(df, x);
pass(1) = (norm(err, 'inf') < tol);

f = funcheb1(@(x) atan(x), pref);
df = diff(f);
df_exact = @(x) 1./(1 + x.^2);
err = df_exact(x) - feval(df, x);
...
f = funcheb1(@(x) x.*sin(x.^2) - 1, pref);
df = diff(f);
g = funcheb1(@(x) exp(-x.^2), pref);
dg = diff(g);
...
const = funcheb1(@(x) ones(size(x)), pref);
dconst = diff(const);
err = feval(dconst, x);
...
f = funcheb1(@(x) x.*atan(x) - x - 0.5*log(1 + x.^2), pref);
df2 = diff(f, 2);
df2_exact = @(x) 1./(1 + x.^2);
...
% DIM option should return an empty funcheb1 for non-vectorized input.
f = funcheb1(@(x) x.^3);
dim2df = diff(f, 1, 2);
