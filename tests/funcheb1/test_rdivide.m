% Test file for funcheb1/rdivide.
...
%%
% Check division of a scalar by a funcheb1 object.

f_op = @(x) exp(x);
f = funcheb1(@(x) exp(x), pref);
pass(7) = test_div_scalar_by_function(alpha, f, f_op, x);

%%
% Check division of two funcheb1 objects.
...
f = funcheb1(@(x) sin(x), pref);
h1 = f ./ alpha;
h2 = funcheb1(@(x) sin(x) ./ alpha, pref);
pass(15) = norm(h1.values - h2.values, 'inf') < tol;

g = funcheb1(@(x) exp(x), pref);
h1 = f ./ g;
h2 = funcheb1(@(x) sin(x) ./ exp(x), pref);
