...
%%
% Check operation for scalar funcheb1 objects.
...
% Multiply non-scalar double and funcheb1.
try
    f = funcheb1(@(x) exp(x));
    disp([1 2 3]*f)
catch ME
...
% Multiply funcheb1 and non-sclar double with mismatching dimensions.
try
...
% Using * for multiplication of two funcheb1 objects.
try
    g = funcheb1(@(x) x);
    disp(f*g);
    pass(11) = false;
...
% Using * to multiply a funcheb1 and something else.
try
    disp(f*uint8(128));
    pass(12) = false;
catch ME
