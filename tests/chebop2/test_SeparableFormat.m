...
N = chebop2(@(x,y,u) diff(u,2,1) + diff(u,2,2) + (10+y).*u, [-1 pi/3 -sqrt(3)/2 1.1]); 
[U, S, V] = SeparableFormat( N );
pass(6)  = checkCoeffs(N.coeffs, U, S, V);

% Variable coefficients on higher derivatives: 
N = chebop2(@(x,y,u) (1+x.^2).*diff(u,2,1) + diff(u,2,2) + u, [-1 pi/3 -sqrt(3)/2 1.1]); 
[U, S, V] = SeparableFormat( N );
...
hb = 0.0256; 
N = chebop2(@(x,y,u) 1i*hb*diff(u,1,1) + hb^2*diff(u,2,2) - x.^2.*u, [-1 1 0 10]); 
[U, S, V] = SeparableFormat( N );
