...
pass(1) = (max([err_x err_y]) < eps*f.smoothPart.vscale);
...
err_y = y(1) - y_exact(1);
err_x = x - x_exact; 
pass(2) = (norm(err_x, inf) < 1e1*eps*f.smoothPart.vscale &&...
    abs(err_y) < 1e4*eps*f.smoothPart.vscale) && (y(2) == Inf);
...
y_err = y - y_exact;
x_err = x - x_exact;
pass(3) = (norm(y_err, inf) < 1e2*eps*f.smoothPart.vscale &&...
    norm(x_err, inf) < 1e2*eps*f.smoothPart.vscale);
...
y_err = y - y_exact;
x_err = x - x_exact;
pass(4) = (norm(y_err, inf) < 1e2*eps*f.smoothPart.vscale &&...
    norm(x_err, inf) < 1e3*eps*f.smoothPart.vscale);
...
err_y = y(2) - y_exact(2);
err_x = x - x_exact; 
pass(5) = (norm(err_x, inf) < 10*eps*f.smoothPart.vscale &&...
    abs(err_y) < 10*eps*f.smoothPart.vscale) && (y(1) == -Inf);
