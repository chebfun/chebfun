...
pass = [];
w = u/2;
pass(length(pass) + 1) = w.isConstant == 1;

w = u./(x+2);
pass(length(pass) + 1) = w.isConstant == 1;

w = sin(u)./(x+2);
pass(length(pass) + 1) = w.isConstant == 0;
...
v = seed(v, 2, 2);
w = u/2;
pass(length(pass) + 1)  = all( w.isConstant == [1 1]);

w = u./(x+2);
pass(length(pass) + 1)  = all( w.isConstant == [1 1]);

w = sin(u)/2;
pass(length(pass) + 1)  = all( w.isConstant == [0 1]);
...
w = u./v;
pass(length(pass) + 1)  = all( w.isConstant == [0 0]);
