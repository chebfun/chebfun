% Test file for fourtech/coeffs2vals.m
...
x_pts = fourtech.fourpts(numel(c));
vTrue = f_exact(x_pts);
...
v = fourtech.coeffs2vals(1i*c);
pass(4) = norm(v - 1i*vTrue, inf) < tol;
pass(5) = ~any(real(v));

%%
% Test general branch
v = fourtech.coeffs2vals((1+1i)*c);
pass(6) = norm(v - (1+1i)*vTrue, inf) < tol;

%%
% Test for array input
v = fourtech.coeffs2vals([c, -c]);
