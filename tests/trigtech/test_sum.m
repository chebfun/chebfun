...
pass(1) = (abs(sum(f) - 0.532131755504017) < 10*f.vscale.*eps);

f = testclass.make(@(x) 3./(4 - cos(pi*x)), [], pref);
pass(2) = (abs(sum(f) - 1.549193338482967) < 10*f.vscale.*eps);

f = testclass.make(@(x) 1+cos(1e4*pi*x), [], pref);
exact = 2;
pass(3) = (abs(sum(f) - exact)/exact < 100*f.vscale.*eps);

f = testclass.make(@(x) 1 + 1i*cos(40*pi*x), [], pref);
exact = 2;
pass(4) = (abs(sum(f) - exact)/exact < 10*f.vscale.*eps);
...
I = sum(f);
I_exact = [0 2 0];
pass(9) = (max(abs(I - I_exact)) < 10*max(f.vscale.*eps));
