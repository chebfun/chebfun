...
D = collocclass.diffmat(n, 1);
df = D*v;
df_exact = @(x) -pi*sin(pi*x).*exp(cos(pi*x));
err = df_exact(x) - df;
pass(1) = ( norm(err, inf) < 1e3*f.vscale.*eps );
...
err = df_exact(x) - df;
pass(2) = ( norm(err, inf) < 1e7*f.vscale.*eps );
...
err = df_exact(x) - df;
pass(3) = ( norm(err, inf) < 1e3*f.vscale.*eps );
...
err = df_exact(x) - df;
pass(4) = ( norm(err, inf) < 1e5*f.vscale.*eps );
...
err = df2_exact(x) - df2;
pass(5) = ( norm(err, inf) < 5e5*f.vscale.*eps );
...
df2 = D2*v;
err = df2_exact(x) - df2;
pass(6) = ( norm(err, inf) < 5e5*f.vscale.*eps );
...
err = df5_exact(x) - df5;
pass(7) = ( norm(err, inf) < 1e3*f.vscale.*eps );
...
df5 = D5*v;
err = df5_exact(x) - df5;
pass(8) = ( norm(err, inf) < 1e3*f.vscale.*eps );
...
err = df6_exact(x) - df6;
pass(9) = ( norm(err, inf) < 1e4*f.vscale.*eps );
...
df6 = D6*v;
err = df6_exact(x) - df6;
pass(10) = ( norm(err, inf) < 1e5*f.vscale.*eps );
