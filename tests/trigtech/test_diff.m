...
pass(2) = (norm(err, inf) < 1e3*df.vscale.*eps);
...
err = df_exact(x) - feval(df, x);
pass(3) = (norm(err, inf) < 1e4*df.vscale.*eps);
...
err = df_exact(x) - feval(df, x);
pass(4) = (norm(err, inf) < 100*df.vscale.*eps);
...
err = df_exact(x) - feval(df, x);
pass(5) = (norm(err, inf) < 1e3*df.vscale.*eps);
...
err = diff(f) - df;
pass(6) = (norm(err.coeffs, inf) < 100*df.vscale.*eps);
...
pass(10) = (norm(err, inf) < 1e3*df2.vscale.*eps);
...
pass(13) = (norm(err(:), inf) < 100*max(df.vscale.*eps));
...
pass(14) = isequal(size(dim2df.vscale), [1 2]) && ...
    (norm(err(:), inf) < 100*max(dim2df.vscale.*eps));
...
pass(15) = isequal(size(dim2df2.vscale), [1 1]) && ...
    (norm(err(:), inf) < 1e3*max(dim2df2.vscale.*eps));
