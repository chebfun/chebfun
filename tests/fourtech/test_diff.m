...
err = df_exact(x) - feval(df, x);
pass(2) = (norm(err, inf) < 10*df.vscale.*df.epslevel);
...
err = diff(f) - df;
pass(5) = (norm(err.coeffs, inf) < 100*df.vscale.*df.epslevel);
...
err = feval(errfn, x);
pass(6) = (norm(err, inf) < max(tol_f, tol_g));

errfn = diff(f.*g) - (f.*dg + g.*df);
err = feval(errfn, x);
pass(7) = (norm(err, inf) < length(f)*max(tol_f, tol_g));

const = testclass.make(@(x) ones(size(x)), [], pref);
dconst = diff(const);
err = feval(dconst, x);
pass(8) = (norm(err, inf) == 0);
...
pass(9) = (norm(err, inf) < 10*df2.vscale.*df2.epslevel);
...
pass(11) = (norm(err, inf) < 10*df5.vscale.*df5.epslevel);
...
pass(12) = (norm(err(:), inf) < 10*max(df.vscale.*df.epslevel));

% DIM option.
dim2df = diff(f, 1, 2);
g = @(x) [(sin(4*pi*(x-0.2))-exp(-50*x.^2))  (1i*exp(cos(pi*x))-sin(4*pi*(x-0.2)))];
err = feval(dim2df, x) - g(x);
pass(13) = isequal(size(dim2df.vscale), [1 2]) && ...
