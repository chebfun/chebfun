% Test file for fourtech/compose.m
...
% Compose g(f), when f and g are FOURTECH objects:
f = testclass.make(@(x) sin(pi*x));
g = testclass.make(@(x) exp(cos(pi*x)));
h = compose(f, g); 
hvalues = h.coeffs2vals(h.coeffs);
x = testclass.fourpts(length(h));
pass(7) = norm(hvalues - exp(cos(pi*sin(pi*x))), inf) < 10*h.vscale.*h.epslevel;

% Compose g(f), when f and g are FOURTECH objects and g is array-valued:
...
% Compose g(f), when f and g are FOURTECH objects and f is array-valued:
...
% We cannot expect to compose two array-valued FOURTECH objects f(g):
try 
    ...
    pass(10) = strcmp(ME.identifier, 'CHEBFUN:FOURTECH:compose:arrval');
end

% We cannot expect to compose two array-valued FOURTECH objects in this way:
try 
    ...
    pass(11) = strcmp(ME.identifier, 'CHEBFUN:FOURTECH:compose:dim');
end
