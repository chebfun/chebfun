...
map = @(x) (x+1)*50;
f = funcheb2(@(x) besselj(0, map(x)), pref);
r = map(roots(f));
...
f = funcheb2(@(x) [sin(pi*x), cos(pi*x)], pref);
r = roots(f);
r2 = [-1 0 1 -.5 .5 NaN].';
