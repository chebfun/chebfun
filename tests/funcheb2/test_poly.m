...
f = funcheb2(@(x) 2i*x.^5 - 3.2*x.^4 + 2*x.^2 - (1.2 + 3i), pref);
p = poly(f);
pass(3) = (norm(p - [2i (-3.2) 0 2 0 -(1.2 + 3i)], 'inf') < 10*f.epslevel);

%%
% Verify operation for vectorized funcheb2 objects.

f = funcheb2(@(x) [3*ones(size(x)), (6.4*x - 3i), (4*x.^2 - 2i*x + 3.7)], pref);
p = poly(f);
p_exact = [0 0     3;
           0 6.4   (-3i);
	   4 (-2i) 3.7];
