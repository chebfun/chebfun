...
for n = 1:2
    ...
    p = trigcoeffs(f);
    pass(n,1) = (norm(p, inf) <= 10*f.vscale.*eps);

    f = testclass.make(@(x) 3*ones(size(x)), [], pref);
    p = trigcoeffs(f);
    pass(n,2) = (norm(p - 3, inf) < 10*f.vscale.*eps);

    f = testclass.make(@(x) 1+cos(pi*x), [], pref);
    p = trigcoeffs(f,3);
    pass(n,3) = (norm(p - [0.5 1 0.5]', inf) < 10*f.vscale.*eps);
    p = trigcoeffs(f,5);
    pass(n,4) = (norm(p - [0 0.5 1 0.5 0]', inf) < 10*f.vscale.*eps);
    p = trigcoeffs(f,1);
    pass(n,5) = (norm(p - 1, inf) < 10*f.vscale.*eps);
    ...
    p_exact = [0 0   0;...
               ...
               0 0.5 0;...
               0 0   1];
    pass(n,9) = (norm(p(:) - p_exact(:), inf) < 10*max(f.vscale.*eps));
    ...
    p_exact = [0 0   0;...
               ...
               0 0   1;...
               0 0   0];
    pass(n,10) = (norm(p(:) - p_exact(:), inf) < 10*max(f.vscale.*eps));

    p = trigcoeffs(f,3);
    p_exact = [0 0.5 1;...   
               3 1   1;...
               0 0.5 0];
    pass(n,11) = (norm(p(:) - p_exact(:), inf) < 10*max(f.vscale.*eps));
