...
for n = 1:2
    ...
    err = diff(f) - df;
    pass(n, 5) = (norm(err.coeffs, inf) < 100*df.vscale.*eps);
    ...
    pass(n, 9) = (norm(err, inf) < 1e6*df2.vscale.*eps);
    ...
    pass(n, 12) = (norm(err(:), inf) < 1e2*max(df.vscale.*eps));
