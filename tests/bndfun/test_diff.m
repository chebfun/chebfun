...
pass(1) = (norm(err, inf) < 1e3*get(f, 'vscale')*get(f, 'epslevel'));

f = bndfun(@(x) atan(x), struct('domain', dom), pref);
df = diff(f);
df_exact = @(x) 1./(1 + x.^2);
err = df_exact(x) - feval(df, x);
pass(2) = (norm(err, inf) < 1e3*get(f, 'vscale')*get(f, 'epslevel'));

f = bndfun(@(x) sin(x), struct('domain', dom), pref);
df = diff(f);
df_exact = @(x) cos(x);
err = df_exact(x) - feval(df, x);
pass(3) = (norm(err, inf) < 1e3*get(f, 'vscale')*get(f, 'epslevel'));
...
err = df_exact(x) - feval(df, x);
pass(4) = (norm(err, inf) < 1e3*get(f, 'vscale')*get(f, 'epslevel'));
...
err = diff(f) - df;
pass(5) = (get(err, 'vscale') < 1e3*get(f, 'vscale')*get(f, 'epslevel'));
...
dconst = diff(const);
err = feval(dconst, x);
pass(8) = (norm(err, inf) <= 10*get(dconst, 'vscale')*get(dconst, 'epslevel'));
...
pass(11) = (norm(err, inf) <= get(df6, 'vscale')^6*get(df6, 'epslevel'));
