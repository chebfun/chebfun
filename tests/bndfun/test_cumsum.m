...
pass(1) = (norm(err, inf) < 5*f.onefun.vscale*tol) && (abs(feval(F, a)) < f.onefun.vscale*tol);

f = bndfun(@(x) 1./(1 + x.^2), dom, [], [], pref);
F = cumsum(f);
F_ex = @(x) atan(x)-atan(a);
err = feval(F, x) - F_ex(x);
pass(2) = (norm(err, inf) < 5*f.onefun.vscale*tol) && (abs(feval(F, a)) < f.onefun.vscale*tol);

f = bndfun(@(x) cos(1e4*x), dom, [], [], pref);
F = cumsum(f);
F_ex = @(x) (sin(1e4*x)-sin(1e4*a))/1e4;
err = feval(F, x) - F_ex(x);
pass(3) = (norm(err, inf) < f.onefun.vscale*tol) && (abs(feval(F, a)) < f.onefun.vscale*tol);
...
err = feval(F, x) - F_ex(x);
pass(4) = (norm(err, inf) < 10*f.onefun.vscale*tol) && (abs(feval(F, a)) < f.onefun.vscale*tol);
...
G = cumsum(f);
err = G - F;
pass(5) = (std(err.onefun.values) < 10*f.onefun.vscale*tol) && (abs(feval(G, a)) < f.onefun.vscale*tol);
...
pass(7) = (norm(err, inf) < 30*g.onefun.vscale*f.onefun.vscale*tol) && (abs(feval(h, a)) < g.onefun.vscale*f.onefun.vscale*tol);
...
pass(8) = (norm(err, inf) < 2*max(f.onefun.vscale)*tol) && all(abs(feval(F, a) < 2*max(f.onefun.vscale)*tol));
...
pass(9) = (norm(err, inf) < 5*(f.onefun.vscale)^2*tol) && abs(feval(F2, a) < (f.onefun.vscale)^2*tol);

F3_exact = bndfun(@(x) cos(x)+x.^2*cos(a)/2+x*(sin(a)-a*cos(a)) - ...
    (cos(a)/2-sin(a)), dom, [], [], pref);
F3 = cumsum(f,3);
err = std(feval(F3, x) - feval(F3_exact, x));
pass(10) = (norm(err, inf) < (f.onefun.vscale)^3*tol) && abs(feval(F3, a) < (f.onefun.vscale)^3*tol);
