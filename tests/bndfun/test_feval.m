...
f_exact = @(x) exp(x) - 1;
pass(1) = (norm(feval(f, x) - f_exact(x), inf) < 2*get(f, 'vscale')*f.onefun.epslevel);

f = bndfun(@(x) 1./(1 + x.^2), dom, [], [], pref);
f_exact = @(x) 1./(1 + x.^2);
pass(2) = (norm(feval(f, x) - f_exact(x), inf) < get(f, 'vscale')*f.onefun.epslevel);
...
f_exact = @(t) sinh(t*z);
pass(4) = (norm(feval(f, x) - f_exact(x), inf) < 2*get(f, 'vscale')*f.onefun.epslevel);
...
pass(5) = (all(size(err) == [1 1000])) && (norm(err(:), inf) < ...
    2*get(f, 'vscale')*f.onefun.epslevel);
...
err = feval(f, x) - f_exact(x);
pass(8) = all(max(abs(err)) < 2*get(f, 'vscale')*f.onefun.epslevel);
...
pass(9) = all(all(abs(fx - f_exact) < max(get(f, 'vscale'))*f.onefun.epslevel));
