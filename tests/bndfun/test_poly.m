...
p = poly(f);
pass(1) = (norm(p, inf) <= get(f, 'vscale')*f.onefun.epslevel);

f = bndfun(@(x) 3*ones(size(x)), dom, [], [], pref);
p = poly(f);
pass(2) = (norm(p - 3, inf) < get(f, 'vscale')*f.onefun.epslevel);

f = bndfun(@(x) 6.4*x - 3i, dom, [], [], pref);
p = poly(f);
pass(3) = (norm(p - [6.4 (-3i)], inf) < get(f, 'vscale')*f.onefun.epslevel);

f = bndfun(@(x) 2i*x.^5 - 3.2*x.^4 + 2*x.^2 - (1.2 + 3i), dom, [], [], pref);
p = poly(f);
pass(4) = (norm(p - [2i (-3.2) 0 2 0 -(1.2 + 3i)], inf) ...
    < get(f, 'vscale')*f.onefun.epslevel);
...
pass(5) = (norm(p(:) - p_exact(:), inf) < ...
    max(get(f, 'vscale'))*f.onefun.epslevel);
