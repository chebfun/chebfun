...
Exact = ballfun(@(r,lam,th)0.5*sqrt(1/pi)*Normalization);
...
m = -1; l = 1;
Normalization = sqrt(2*l+3);
Exact = ballfun(@(r,lam,th)0.5*sqrt(3/(2*pi))*r.^l.*exp(1i*m*lam).*sin(th)*Normalization);
f = ballfun.solharm(l,m);
...
m = 0; l = 1;
Normalization = sqrt(2*l+3);
Exact = ballfun(@(r,lam,th)0.5*sqrt(3/pi)*r.^l.*exp(1i*m*lam).*cos(th)*Normalization);
f = ballfun.solharm(l,m);
...
m = -2; l = 2;
Normalization = sqrt(2*l+3);
Exact = ballfun(@(r,lam,th)0.25*sqrt(15/(2*pi))*r.^l.*exp(1i*m*lam).*sin(th).^2*Normalization);
f = ballfun.solharm(l,m);
...
m = -1; l = 2;
Normalization = sqrt(2*l+3);
Exact = ballfun(@(r,lam,th)0.5*sqrt(15/(2*pi))*r.^l.*exp(1i*m*lam).*sin(th).*cos(th)*Normalization);
f = ballfun.solharm(l,m);
...
m = 0; l = 2;
Normalization = sqrt(2*l+3);
Exact = ballfun(@(r,lam,th)0.25*sqrt(5/pi)*r.^l.*exp(1i*m*lam).*(3*cos(th).^2-1)*Normalization);
f = ballfun.solharm(l,m);
