...
% If funIn only has one input argument, we just give it a treeVar() argument.
...
% Return the potentially new breakpoints we obtained during the evaluation of
% problemFun:
problemDom = problemFun.domain;

% In the scalar case, varIndex will always be 1, as only one variable is
% involved:
varIndex = 1;
...
% If newTree is empty, we only have a derivative part in the expression, e.g.
% diff(u) = 0. We must replace it with a 0, as otherwise, we can't evaluate the
% resulting odeFun in the ODE solvers.
if ( isempty(newTree) )
    newTree = 0;
end
