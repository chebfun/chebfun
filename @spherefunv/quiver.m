...
p = (1+sqrt(5))/2;
x = [[0,p,1];[0,-p,1];[0,p,-1];[0,-p,-1];[1,0,p];[-1,0,p];[1,0,-p];...
                     [-1,0,-p];[p,1,0];[-p,1,0];[p,-1,0];[-p,-1,0]];
...
% TRISECTTRI    Subdivide triangular mesh into triangular trisection 
%    TRISECTTRI( X, TRI ) Subdivide triangular mesh using triangular 
%    trisection. During this
...
%   [X, TRI] = BISECTTRI(X, TRI )  Subdivide triangular mesh using 
%   triangular bisection. During this operation vertices are inserted at 
%   the bisection points of the triangles edges. This produces nine new 
%   triangular faces for every face of the original triangle.
...
%TRISECTEDGE   Trisect a great circle by an arc
%    X = TRISECTEDGE(X1, X2) Trisects the great circle arc joining X1 and X2.
...
%CIRCUMCENTERSPHTRI   Calculate circumcenter of scribed circle 
% 
%  XC = CIRCUMCENTERSPHTRI(TRI, NODES) 

% TODO: Improve documentation of this subfunction. Add inline comments. 
