...
elseif ( nargin == 2 )
    ...
    if ( isa( c, 'chebfun' ) ) 
        ...
        if ( ~isreal( c ) )
            ...
            % this is AT's choice. AT's choice treats both variables
            % equally, but is very slow. If you don't treat variables
            % equally it may be fast. 
            % 
            ...
            % selected op (below) so that M = -op*y and L = op*x.) There is
            % a lot of freedom in choosing M and L. It may be better to
            % pick M and L that are easier to construct. 
