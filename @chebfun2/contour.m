...
% X and Y are matrices that are used to specify the plotting grid.
%
...
elseif ( isa(f,'chebfun2') )   
    dom = f.domain;
    if ( ( nargin == 3 ) || ( ( nargin > 3 ) && ( ~isa(argin{1},'chebfun2') ) ) )  % contour(xx, yy, f)
    ...
    elseif ( ( nargin >= 3 ) && ( isa(argin{1},'chebfun2') ) && ...  % contour plot on a surface.
                                                  ( isa(argin{2},'chebfun2') ) )
        % Extract inputs:
        xx = f; 
        yy = argin{1}; 
        ...
        % contour plot:
        [c, h] = contour( xx, yy, vals, argin{3:end} );
    elseif ( ( nargin == 1) || ( ( nargin > 1 ) && ( isa(argin{1},'double') ) ) )    % contour(f) 
