...
%MAT2CELL   Convert an array-valued FOURIETECH into a cell array of FOURIETECH
%objects.
%
%   G = MAT2CELL(F, C) breaks up the array-valued FOURIETECH F into a single row
%   cell array G of FOURIETECH objects. C is the vector of column sizes, and if F
...
%
% Example:
%   f = fourier.constructor(@(x) [sin(x), cos(x), exp(cos(x)), sin(sin(x)])
...
if ( ~isscalar(M) || (M ~= 1) )
    error('FOURIETECH:FOURIETECH:mat2cell:size', ...
...
for k = 1:numel(N)
    ...
    % Assign values to the fields of the FOURIETECH
