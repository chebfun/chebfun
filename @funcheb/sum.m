...
%SUM    Definite integral of a FUNCHEB on the interval [-1,1].
%   SUM(F) is the integral from -1 to 1 of F.
%
%   If F is a vector-valued FUNCHEB then the result is a row vector containing
%   the definite integrals of each column.
%
%   SUM(F, 2) sums over the second dimension of F, i.e., adds up its columns. If
%   F is a scalar-valued FUNCHEB, this simply returns F.
%
...
%%
% Sum across vectorised FUNCHEB columns if dim = 2:
if ( nargin > 1 && dim == 2 )
...
% Trivial cases:
if ( isempty(f) )    % Empty FUNCHEB
    out = []; 
    return
elseif ( n == 1 )    % Constant FUNCHEB
    out = 2*f.values;
    return
end
