...
%   an array of FUNCHEB objects, where the entries hold F restricted to each of
%   the subintervals defined by S.
%
%   If F is a vector-valued function, say [F1, F2], then the restrict(F, S =
%   [S1, S2, S3]) returns the vector-valued FUNCHEB {restrict(F1,S).
%   restrict(F2, S)}.
%
...
if ( s(1) < -1 || s(end) > 1 || any(diff(s) <= 0) )
    error('FUNCHEB:restrict:badinterval', 'Not a valid interval.')
