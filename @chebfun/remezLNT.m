...
while ( (abs(abs(h)-abs(err)) > opts.tol) && ...
...
if ( abs(abs(h)-abs(err)) > opts.tol && dialogFlag)
...
err_handle = @(x) feval(f, x) - rh(x);
sample_points = linspace(f.domain(1),f.domain(end),100);
...
% Initial trial
if ( isempty(xk) )
    ek = chebfun(@(x) err_handle(x), doms.', 'eps', reltol, 'splitting', 'on');
