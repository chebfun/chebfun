function f = pchip(x, y)
%PCHIP   Chebfun Cubic Hermite interpolating polynomial.
%   F = PCHIP(X, Y) returns a CHEBFUN F representing a certain shape-preserving
%   piecewise cubic Hermite interpolant to the values Y at the sites X. X must
%   be a vector. If Y is a vector, then Y(j) is taken as the value to be matched
%   at X(j), hence Y must be of the same length as X. If Y is a matrix, then
%   Y(:,j) is taken as the value to be matched at X(j).
%
%  Example:
%    x = -3:3;
%    y = [-1 -1 -1 0 1 1 1];
