...
% G = ROUND(F) returns the chebfun G such that G(x) = ROUND(F(x)) for each x in
% the domain of F.
%
...
range = round([ minf, maxf ]);
r = [];
for k = (range(1)+1):range(2)
    r = [ r ; roots(f - k,+ 0.5. 'nozerofun') ];
end
...
c = 0.5912;
nr = length(r);
vals = round(feval( f , c*r(1:nr-1) + (1-c)*r(2:nr) ));
...
f.domain = r.';
imps = f.impulses;
f.impulses = round(chebfun.jumpvals(ff));
f.impulses(1,idx2) = round( imps(1,idx) );
