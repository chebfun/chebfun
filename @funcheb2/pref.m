...
%PREF Prefence settings for FUNCHEB2.
% FUNCHEB2.PREF(PREFNAME) returns the value corresponding to the preference
% named in the string PREFNAME.
%
% P = FUNCHEB2.PREF returns a structure P with a field P.FUNCHEB2 which contains
% the default FUNCHEB2 preferences as fields/values. This structure may be
% passed to the FUNCHEB2 constructor.
%
% P = FUNCHEB2.PREF(P) will check to see whether the input preference structure
% P already has a FUNCHEB2 field. If it does not, one is appended.
%
% P = FUNCHEB2.PREF('PREFNAME1', VAL1, 'PREFNAME1', VAL2, ...) returns the same
% structure as above, but with the default FUNCHEB2 preferences 'PREFNAME1',
% 'PREFNAME1', replaced by the values in VAL1, VAL2, etc.
%
% P = FUNCHEB2.PREF(P, 'PREFNAME1', VAL1, 'PREFNAME1', VAL2, ...) appends a
% FUNCHEB2 preference field to P if required, and modifies the FUNCHEB2
% properties 'PREFNAME1' and 'PREFNAME1'.
%
% Note that no checking of either the input PREFNAMEs or VALs takes place.
%
% FUNCHEB2 PREFERENCES (case sensitive)
%
%     eps          -  Relative tolerance used in construction and subsequent
%      [2^-52]        operations. See FUNCHEB2.HAPPINESSCHECK for more details.
%
%     extrapolate
...
%        [1]          horizontal scale invariance when using the FUNCHEB2
...
%
% See also FUNCHEB2, FUNCHEB2.extrapolate.
