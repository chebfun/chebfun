...
%ROOTS	Roots of a FUNCHEB2 in the interval [-1,1].
%   ROOTS(F) returns the real roots of the FUNCHEB2 F in the interval [-1,1].
%
...
%     double
%
%   If F is a vector-valued FUNCHEB2 then there is no reason to expect each
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ROOTS works by recursively bisecting the interval until the resulting funcheb2
...
if ( rootspref.prune && ~rootspref.recurse )
    ...
    % c on the unit interval.
        ...
        % Otherwise, split using more traditional methods
            ...
            v = funcheb2.clenshaw( [ chebptsAB(n, [-1, splitpoint]) ; chebptsAB(n, [splitpoint, 1]) ], c(end:-1:1) );
