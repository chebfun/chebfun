...
% Compress U and V:
if ( size(allCols,2) > 1 )
    [Su, ~, Vu, colsTucker] = completeACA2D(allCols, globalTolCols, 0);
...
if ( size(allRows,2) > 1 )
    [Sv, ~, Vv, rowsTucker] = completeACA2D(allRows, globalTolRows, 0);
    Vv = Vv*diag(1./Sv);
else
    rowsTucker = allRows; 
    Vv = 1;
...
%%
function absTol = GetTol2Dv2(xx, yy, vals, dom, pseudoLevel)
